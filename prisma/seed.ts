...
import {
  ...
  CategorySeed,
  categorySeeds,
  ccBuilderComponentSeeds,
  pcBuilderComponentSeeds,
} from '../src/catalog/initial-catalog';
...
async function upsertCategory(seed: CategorySeed, parentId: string | null, level: number, sortOrder: number, path: string[] = []) {
...
async function upsertDemoProducts() {
  for (const [index, product] of demoProducts.entries()) {
    ...
    const isCcComponent = ['camera', 'recorder', 'storage', 'power-source', 'connector', 'cable', 'display', 'router'].includes(product.componentType ?? '');
    ...
    await prisma.product.upsert({
      ...
      create: {
        ...
        specMeta: { create: Object.entries(product.meta ?? {}).map(([key, value]) => ({ key, value })) },
      },
      update: {
        name: product.name,
        categoryId: category.id,
        ...
        componentMaps: {
          deleteMany: {},
          ...(product.componentType ? { create: [{ builderType, componentType: product.componentType }] } : {}),
        },
        specMeta: { deleteMany: {}, create: Object.entries(product.meta ?? {}).map(([key, value]) => ({ key, value })) },
      },
    });
  }
}

const inferDemoVisual = (slug: string) => {
  if (slug.includes('desktop-pc')) return '/uploads/demo/desktop.svg';
  if (slug.includes('network') || slug.includes('router') || slug.includes('access-point') || slug.includes('range-extender') || slug.includes('lan-card')) return '/uploads/demo/network.svg';
  if (slug.includes('laptop') || slug.includes('tablet')) return '/uploads/demo/laptop.svg';
  if (slug.includes('processor')) return '/uploads/demo/cpu.svg';
  if (slug.includes('motherboard')) return '/uploads/demo/motherboard.svg';
  if (slug.includes('monitor') || slug.includes('display') || slug.includes('projector')) return '/uploads/demo/monitor.svg';
  if (slug.includes('ram') || slug.includes('memory')) return '/uploads/demo/ram.svg';
  if (slug.includes('ssd') || slug.includes('hdd') || slug.includes('storage') || slug.includes('drive')) return '/uploads/demo/storage.svg';
  if (slug.includes('graphics-card')) return '/uploads/demo/gpu.svg';
  if (slug.includes('camera') || slug.includes('dvr') || slug.includes('nvr') || slug.includes('security')) return '/uploads/demo/cctv.svg';
  return '/uploads/demo/accessory.svg';
};

const inferBuilder = (slug: string): { builderType: 'PC' | 'CC_CAMERA'; componentType: string; meta: Record<string, string> } | null => {
  if (slug.includes('processor')) return { builderType: 'PC', componentType: 'cpu', meta: { cpu_socket: 'AM5', socket: 'AM5', generation: 'Demo Gen', power_watt: '65' } };
  if (slug.includes('motherboard')) return { builderType: 'PC', componentType: 'motherboard', meta: { motherboard_socket: 'AM5', socket: 'AM5', motherboard_ram_type: 'DDR5', ram_type: 'DDR5', form_factor: 'ATX', power_watt: '45' } };
  if (slug.includes('ram')) return { builderType: 'PC', componentType: 'ram', meta: { ram_type: 'DDR5', capacity: '16GB', bus_speed: '5600MHz', power_watt: '6' } };
  if (slug.includes('ssd')) return { builderType: 'PC', componentType: 'ssd', meta: { storage_interface: 'NVMe', storage_type: 'SSD/NVMe', power_watt: '6' } };
  if (slug.includes('hdd')) return { builderType: 'PC', componentType: 'hdd', meta: { storage_interface: 'SATA', storage_type: 'HDD/SATA', power_watt: '8' } };
  if (slug.includes('graphics-card')) return { builderType: 'PC', componentType: 'graphics-card', meta: { gpu_power_requirement: '250', required_watt: '250', interface: 'PCIe 4.0', power_watt: '250' } };
  if (slug.includes('monitor') || slug.includes('display')) return { builderType: 'PC', componentType: 'monitor', meta: { power_watt: '35' } };
  if (slug.includes('keyboard')) return { builderType: 'PC', componentType: 'keyboard', meta: { power_watt: '3' } };
  if (slug.includes('mouse')) return { builderType: 'PC', componentType: 'mouse', meta: { power_watt: '2' } };
  if (slug.includes('casing-fan')) return { builderType: 'PC', componentType: 'casing-fan', meta: { power_watt: '4' } };
  if (slug.includes('casing')) return { builderType: 'PC', componentType: 'casing', meta: { form_factor: 'ATX/mATX/ITX', supported_form_factor: 'ATX/mATX/ITX', power_watt: '0' } };
  if (slug.includes('power-supply')) return { builderType: 'PC', componentType: 'power-supply', meta: { psu_watt: '750', watt: '750', power_watt: '0' } };
  if (slug.includes('cpu-cooler')) return { builderType: 'PC', componentType: 'cpu-cooler', meta: { power_watt: '10' } };
  if (slug.includes('ups')) return { builderType: 'PC', componentType: 'ups', meta: { power_watt: '0' } };
  if (slug.includes('optical')) return { builderType: 'PC', componentType: 'optical-device', meta: { power_watt: '15' } };
  if (slug.includes('ip-camera') || slug.includes('cc-camera') || slug.includes('webcam') || slug.includes('smart-door-bell')) return { builderType: 'CC_CAMERA', componentType: 'camera', meta: { camera_type: slug.includes('cc-camera') ? 'Analog' : 'IP', resolution: '2MP', power_type: slug.includes('cc-camera') ? '12V' : 'PoE', power_watt: '7' } };
  if (slug.includes('dvr')) return { builderType: 'CC_CAMERA', componentType: 'recorder', meta: { recorder_type: 'Analog', channel_support: '4 Channel', storage_support: 'SATA', power_watt: '14' } };
  if (slug.includes('nvr')) return { builderType: 'CC_CAMERA', componentType: 'recorder', meta: { recorder_type: 'IP', channel_support: '8 Channel', storage_support: 'SATA', power_watt: '18' } };
  if (slug.includes('network-cable') || slug.includes('cable-lan')) return { builderType: 'CC_CAMERA', componentType: 'cable', meta: { cable_type: 'IP/Cat6', power_watt: '0' } };
  if (slug.includes('connector')) return { builderType: 'CC_CAMERA', componentType: 'connector', meta: { power_watt: '0' } };
  if (slug.includes('router')) return { builderType: 'CC_CAMERA', componentType: 'router', meta: { power_watt: '12' } };
  return null;
};
...
async function upsertBulkDemoProducts() {
  ...
  const allowedRootSlugs = new Set(categorySeeds.map((category) => slugify(category.name)));
  const rootSlugFor = (category: (typeof categories)[number]) => {
  ...
  for (let index = 0; index < 200; index += 1) {
    ...
    await prisma.product.upsert({
      ...
      create: {
        ...
        specMeta: { create: Object.entries(builder?.meta ?? { power_watt: '5' }).map(([key, value]) => ({ key, value })) },
      },
      update: {
        name: productName,
        categoryId: category.id,
        ...
        componentMaps: { deleteMany: {}, ...(builder ? { create: [{ builderType: builder.builderType, componentType: builder.componentType }] } : {}) },
        specMeta: { deleteMany: {}, create: Object.entries(builder?.meta ?? { power_watt: '5' }).map(([key, value]) => ({ key, value })) },
      },
    });
  }
}
...
async function main() {
  ...
  for (const [index, zone] of branchZoneSeeds.entries()) {
  ...
  for (const [index, component] of pcBuilderComponentSeeds.entries()) {
  ...
  for (const [index, component] of ccBuilderComponentSeeds.entries()) {
    await prisma.builderComponent.upsert({
      where: { builderType_slug: { builderType: 'CC_CAMERA', slug: component.slug } },
      create: {
        ...component,
        builderType: 'CC_CAMERA',
        sortOrder: index,
        allowMultiple: component.allowMultiple ?? false,
      },
      update: {
